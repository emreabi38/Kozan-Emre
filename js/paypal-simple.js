...
function showPayPalMeSuccess(planType, details) {
    ...
    const depositAmount = planType === 'basic' ? 49.99 : (planType === 'standard' ? 74.99 : 124.99);
    
    // PayPal.me Link
    const paypalMeLink = `https://www.paypal.com/paypalme/kozan885/9.99`;
    ...
    const successHTML = `
        <div style="background: rgba(0,255,0,0.1); border: 2px solid #00ff00; border-radius: 15px; padding: 30px; margin: 20px 0;">
            ...
            <div style="background: rgba(0,0,0,0.3); border-radius: 10px; padding: 20px; margin: 20px 0;">
                ...
                <p><strong>Anzahlung: ${depositAmount.toFixed(2)}€ ✅</strong></p>
                <p><strong>Monatlich ab jetzt: ${monthlyAmount.toFixed(2)}€</strong></p>
                    ...
                    <p style="text-align: center;">
                        <a href="${paypalMeLink}" target="_blank" 
                          style="display: inline-block; padding: 12px 25px; background: linear-gradient(135deg, #0070ba, #009cde); color: white; text-decoration: none; border-radius: 8px; font-weight: bold; margin: 10px 0;">
                            <i class="fab fa-paypal"></i> Direkt ${monthlyAmount.toFixed(2)}€ bezahlen
                        </a>
                    </p>
                ...
                <div style="margin-top: 25px; padding: 15px; background: rgba(255,255,255,0.05); border-radius: 8px;">
                    ...
                    <ul style="margin-left: 20px;">
                        ...
                        <li><strong>In 30 Tagen:</strong> Erste monatliche Zahlung (${monthlyAmount.toFixed(2)}€)</li>
                    </ul>
                </div>
        ...
        paypal.Buttons({
            ...
            createOrder: function(data, actions) {
                return actions.order.create({
                    purchase_units: [{
                        description: 'Basic Website - Anzahlung (49.99€)',
                        amount: {
                            value: '49.99',
                            currency_code: 'EUR'
                        }
    ...
    try {
        paypal.Buttons({
            ...
            createOrder: function(data, actions) {
                return actions.order.create({
                    purchase_units: [{
                        description: 'Standard Website - Anzahlung (74.99€)',
                        amount: {
                            value: '74.99',
                            currency_code: 'EUR'
                        }
    ...
    try {
        paypal.Buttons({
            ...
            createOrder: function(data, actions) {
                return actions.order.create({
                    purchase_units: [{
                        description: 'Premium Website - Anzahlung (124.99€)',
                        amount: {
                            value: '124.99',
                            currency_code: 'EUR'
                        }
...
ANZAHLUNG: ${depositAmount.toFixed(2)}€ ✅
MONATLICH: ${monthlyAmount.toFixed(2)}€
...
- ${nextDateFormatted}: Erste monatliche Zahlung (${monthlyAmount.toFixed(2)}€)
...
`;
    ...
    // Email öffnen
    const subject = `NEUE BESTELLUNG: ${details.payer.name.given_name} - ${packageName} - ${monthlyAmount.toFixed(2)}€/Monat`;
...
function logPayment(planType, details) {
    const amount = planType === 'basic' ? 49.99 : (planType === 'standard' ? 74.99 : 124.99);
    const paymentLog = {
        plan: planType,
        amount: amount,
        monthly: 9.99,
