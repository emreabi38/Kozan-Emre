...
function showPayPalMeSuccess(planType, details) {
    const monthlyAmount = planType === 'basic' ? 9.90 : planType === 'standard' ? 14.90 : 19.90;
    const packageName = planType === 'basic' ? 'Basic' : planType === 'standard' ? 'Standard Website' : 'Premium Website';
    const depositAmount = planType === 'basic' ? 99 : planType === 'standard' ? 199 : 349;
    ...
    const successHTML = `
        <div style="background: rgba(0,255,0,0.1); border: 2px solid #00ff00; border-radius: 15px; padding: 30px; margin: 20px 0;">
            ...
            <div style="background: rgba(0,0,0,0.3); border-radius: 10px; padding: 20px; margin: 20px 0;">
                ...
                <p><strong>Anzahlung: ${depositAmount.toFixed(2).replace('.', ',')}€ ✅</strong></p>
                <p><strong>Monatlich ab jetzt: ${monthlyAmount.toFixed(2).replace('.', ',')}€</strong></p>
                    ...
                    <p style="text-align: center;">
                        <a href="${paypalMeLink}" target="_blank" 
                          style="display: inline-block; padding: 12px 25px; background: linear-gradient(135deg, #0070ba, #009cde); color: white; text-decoration: none; border-radius: 8px; font-weight: bold; margin: 10px 0;">
                            <i class="fab fa-paypal"></i> Direkt ${monthlyAmount.toFixed(2).replace('.', ',')}€ bezahlen
                        </a>
                    </p>
                ...
                <div style="margin-top: 25px; padding: 15px; background: rgba(255,255,255,0.05); border-radius: 8px;">
                    ...
                    <ul style="margin-left: 20px;">
                        ...
                        <li><strong>In 2-10 Tagen:</strong> Deine ${packageName} ist fertig 🚀</li>
                        <li><strong>In 30 Tagen:</strong> Erste monatliche Zahlung (${monthlyAmount.toFixed(2).replace('.', ',')}€)</li>
                    </ul>
                </div>
        ...
        paypal.Buttons({
            ...
            createOrder: function(data, actions) {
                return actions.order.create({
                    purchase_units: [{
                        description: 'Basic Website - Anzahlung (99€)',
                        amount: {
                            value: '99.00',
                            currency_code: 'EUR'
                        }
    ...
    try {
        paypal.Buttons({
            ...
            createOrder: function(data, actions) {
                return actions.order.create({
                    purchase_units: [{
                        description: 'Standard Website - Anzahlung (199€)',
                        amount: {
                            value: '199.00',
                            currency_code: 'EUR'
                        }
    ...
    try {
        paypal.Buttons({
            ...
            createOrder: function(data, actions) {
                return actions.order.create({
                    purchase_units: [{
                        description: 'Premium Website - Anzahlung (349€)',
                        amount: {
                            value: '349.00',
                            currency_code: 'EUR'
                        }
...
ANZAHLUNG: ${depositAmount.toFixed(2).replace('.', ',')}€ ✅
MONATLICH: ${monthlyAmount.toFixed(2).replace('.', ',')}€
...
- ${nextDateFormatted}: Erste monatliche Zahlung (${monthlyAmount.toFixed(2).replace('.', ',')}€)
...
`;
    ...
    // Email öffnen
    const subject = `NEUE BESTELLUNG: ${details.payer.name.given_name} - ${packageName} - ${monthlyAmount.toFixed(2).replace('.', ',')}€/Monat`;
...
function logPayment(planType, details) {
    const amount = planType === 'basic' ? 99 : planType === 'standard' ? 199 : 349;
    const monthlyAmount = planType === 'basic' ? 9.90 : planType === 'standard' ? 14.90 : 19.90;
    const paymentLog = {
        plan: planType,
        amount: amount,
