...
function showPayPalMeSuccess(planType, details) {
    const monthlyAmount = planType === 'premium' ? 14.90 : 9.90;
    const packageName = planType === 'basic' ? 'Basic Website' : (planType === 'standard' ? 'Standard Website' : 'Premium Website');
    const depositAmount = planType === 'basic' ? 49 : (planType === 'standard' ? 74.90 : 124.90);
    ...
    const formattedDeposit = planType === 'basic' ? '49' : depositAmount.toFixed(2);
    ...
    const successHTML = `
        <div style="background: rgba(0,255,0,0.1); border: 2px solid #00ff00; border-radius: 15px; padding: 30px; margin: 20px 0;">
            ...
            <div style="background: rgba(0,0,0,0.3); border-radius: 10px; padding: 20px; margin: 20px 0;">
                ...
                <p><strong>Anzahlung: ${formattedDeposit}€ ✅</strong></p>
        ...
        paypal.Buttons({
            ...
            createOrder: function(data, actions) {
                return actions.order.create({
                    purchase_units: [{
                        description: 'Basic Website - Anzahlung (49€)',
                        amount: {
                            value: '49.00',
                            currency_code: 'EUR'
                        }
    ...
    try {
        paypal.Buttons({
            ...
            createOrder: function(data, actions) {
                return actions.order.create({
                    purchase_units: [{
                        description: 'Standard Website - Anzahlung (74.90€)',
                        amount: {
                            value: '74.90',
                            currency_code: 'EUR'
                        }
    ...
    try {
        paypal.Buttons({
            ...
            createOrder: function(data, actions) {
                return actions.order.create({
                    purchase_units: [{
                        description: 'Premium Website - Anzahlung (124.90€)',
                        amount: {
                            value: '124.90',
                            currency_code: 'EUR'
                        }
...
function logPayment(planType, details) {
    const amount = planType === 'basic' ? 49 : (planType === 'standard' ? 74.90 : 124.90);
    const monthlyAmount = planType === 'premium' ? 14.90 : 9.90;
    const paymentLog = {
        plan: planType,
        amount: amount,
